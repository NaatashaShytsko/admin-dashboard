$(document).ready(function () {
    ...
    chart.on('draw', function (data) {
        if (['line', 'area'].indexOf(data.type) !== -1) {
...
var Circle = function (sel) {
    ...
    [].forEach.call(circles, function (el) {
        ...
        if ((document.body.clientWidth <= '1200') && (document.body.clientWidth >= '992')) {
            valEl = valEl * 357 / 100;
            el.innerHTML = '<figure><svg width="140" height="140"><circle transform="rotate(-90)" r="55" cx="-70" cy="70" /><circle transform="rotate(-90)" style=" stroke-dasharray:' + valEl + 'px 357px;" r="55" cx="-70" cy="70" /><text  x="50%" y="50%" alignment-baseline="middle" text-anchor="middle">' + el.innerHTML + '</text></svg><figcaption></figcaption></figure>';
